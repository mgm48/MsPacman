...
public class EatPillsAction implements RulesAction {
	...
	public MOVE execute(Game game) {
		 if (game.isJunction(game.getPacmanCurrentNodeIndex())) {        //if it requires an action
				return game.getApproximateNextMoveTowardsTarget(game.getPacmanCurrentNodeIndex(),closestPillIndex , game.getPacmanLastMoveMade(), DM.PATH);
			 }
		return MOVE.NEUTRAL;
	}
	...
	public void parseFact(Fact actionFact) {
		try {
			Value value = actionFact.getSlotValue("pill");
			if(value == null)
				return;
